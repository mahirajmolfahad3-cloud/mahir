...
export function Hero() {
  return (
    <section id="top" aria-label="Introduction" className="relative overflow-hidden px-3 pb-12 pt-8 sm:px-6 sm:pt-10 lg:px-8 lg:pb-16 lg:pt-12">
      {/* Decorative background elements */}
      <div className="organic-backdrop pointer-events-none absolute inset-0" aria-hidden="true" />
      <div className="hero-dot-grid pointer-events-none absolute inset-0 opacity-[0.03]" aria-hidden="true" />
                ...
                <div className="mt-8 flex flex-wrap gap-2 sm:mt-9">
                  {PROJECT_TAGS.map((item, i) => (
                    <Reveal key={item} delay={i * 50}>
                      <span className="rounded-full border border-border/50 bg-paper-surface/50 px-3.5 py-1.5 text-[11px] font-medium text-ink-muted/80">
                        {item}
                      </span>
                    </Reveal>
                  ))}
